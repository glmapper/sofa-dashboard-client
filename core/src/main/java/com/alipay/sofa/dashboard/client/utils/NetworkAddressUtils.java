...
public final class NetworkAddressUtils {
     ...
     */
    public static void calculate(String enabledIpRange, String bindNetworkInterface) {
        IP_RANGES = new CopyOnWriteArrayList<>();
        // 没有设置bind_network_interface
        if (StringUtils.isEmpty(enabledIpRange)) {
            ...
            String[] ipRanges = enabledIpRange.split(COMMA);
            for (String ipRange : ipRanges) {
                ...
                if (ipRange.indexOf(COLON) > -1) {
                    String[] ranges = ipRange.split(COLON);
    ...
    private static boolean isValidIp(InetAddress ip, boolean useNi) {
        return !ip.isLoopbackAddress() && ip.getHostAddress().indexOf(COLON) == -1
